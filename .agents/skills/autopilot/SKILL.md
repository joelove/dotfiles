...
name: autopilot
description: >-
  Drive a pull request to a merge-ready state by triaging review comments,
  resolving clear conflicts, and fixing CI in a loop. Use when asked to
  autopilot a PR, get a PR green and mergeable, work through review comments,
  or fix failing CI on a branch.
---
...
Review active unresolved comments and review threads, including automated reviewers. When fetching GitHub comments, filter out resolved threads first. Read only each comment body and the minimum location/URL needed to act on it; do not read the entire JSON output or other unnecessary payload data.
